function [chi_sqr,chi_sqr_cc,p_exact,Qa,Qb,Qc,Qd] = ...
    McNemarsScoreSNPExactIn(Allele,ExactIn,diploT_array_sort,SNP_Num)
...
if ExactIn == 1
    for i = 1:numel(control_no_effect)
        ...
        if (case_test == 100 || control_test == 100) % N or 0, no score for animal(s)
            contine  % don't score pair when there is no score for the alleles for one or both animals
